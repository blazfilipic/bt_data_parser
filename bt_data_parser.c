...
struct VendorInfo {
    unsigned char food_temperature_high;
    unsigned char food_temperature_low;
    unsigned char max_possible_temperature_high;
    unsigned char max_possible_temperature_low;
    unsigned char battery_voltage_high;
    unsigned char battery_voltage_low;
};
...
void parse_data(const unsigned char *byte_data, size_t length, struct DataType* data_types, struct Data* data_array, size_t* data_count) {
    ...
    for (int i = 0; data_types[i].type != 0xFF; i++) {
    ...
    if (data_type == 0xFF) {
        ...
        vendor_info->food_temperature_high = usable_data[0];
        vendor_info->food_temperature_low = usable_data[1];
        vendor_info->max_possible_temperature_high = usable_data[2];
        vendor_info->max_possible_temperature_low = usable_data[3];
        vendor_info->battery_voltage_high = usable_data[4];
        vendor_info->battery_voltage_low = usable_data[5];
